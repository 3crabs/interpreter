...
def expression_7():
    ...
    if read_lex().name == 'ROUND_LEFT':
        ...
        if read_lex().name == 'CURLY_LEFT':
            composite_operator()
        elif read_lex().name == 'DEC':
            return int(next_lex().value)
        elif read_lex().name == 'CASE':
            if next_lex().name != 'CASE':
                err('Ожидался case')
            expression()
            if next_lex().name != 'COLON':
                err('Ожидался :')
            while read_lex().name != 'CASE' and read_lex().name != 'DEFAULT':
                next_lex()
        else:
